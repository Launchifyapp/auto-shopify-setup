  ...
  useEffect(() => {
    async function fullSetup() {
      try {
        ...
        const res1 = await authenticatedFetch(`/api/setup-shop?lang=${lang}`);
        const data1 = await res1.json();
        if (!data1.ok) throw new Error(data1.error || t(lang, "errorSetup"));

        setStep(2);
        // 2. Upload theme
        const res2 = await authenticatedFetch(`/api/upload-theme?lang=${lang}`);
        const data2 = await res2.json();
        if (!data2.ok || !data2.themeId) throw new Error(data2.error || t(lang, "errorThemeUpload"));

        setStep(3);
        // 3. Publish theme
        const res3 = await authenticatedFetch(`/api/publish-theme?themeId=${data2.themeId}`);
