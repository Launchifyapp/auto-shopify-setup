...
export async function POST(req: NextRequest) {
  // Récupère les données du body JSON : url, filename, mimeType
  ...
  // Récupère le buffer image depuis l'URL source
  const imageRes = await fetch(url);
  const imageBuffer = Buffer.from(await imageRes.arrayBuffer());
  uploadForm.append("file", new File([imageBuffer], filename, { type: mimeType }));

  // Envoie le fichier vers S3 via staged target
  const uploadRes = await fetch(target.url, {
    method: "POST",
    body: uploadForm,
    // formdata-node gère les headers multipart automatiquement
  });

  if (!uploadRes.ok) {
    return Response.json(
      { ok: false, error: "Erreur upload S3", details: await uploadRes.text() }, 
      { status: 500 }
    );
  }

  // Step 3: fileCreate pour référencer le fichier Shopify
