...
/**
 * Shopify staged upload workflow (mutation → S3 POST → fileCreate)
 */

const SHOP = "monshop.myshopify.com"; // à adapter selon ton .env
...
async function getStagedUploadUrl(shop: string, token: string, filename: string, mimeType: string) {
  ...
  let json: any = null;
  try { json = JSON.parse(bodyText); } catch { throw new Error(`stagedUploadsCreate failed: Non-JSON response (${res.status}) | Body: ${bodyText}`); }
  if (!json?.data?.stagedUploadsCreate?.stagedTargets?.[0]) throw new Error("stagedUploadsCreate returned no stagedTargets: " + JSON.stringify(json));
...
async function uploadToStagedUrl(stagedTarget: any, fileBuffer: Buffer, mimeType: string, filename: string) {
  ...
  if (!res.ok) { const errText = await res.text(); throw new Error(`S3 upload failed: ${res.status} | ${errText}`); }
...
async function fileCreateFromStaged(shop: string, token: string, resourceUrl: string, filename: string, mimeType: string) {
  ...
  const bodyText = await res.text();
  let json: any = null;
  try { json = JSON.parse(bodyText); } catch { throw new Error(`fileCreate failed: Non-JSON response (${res.status}) | Body: ${bodyText}`); }
