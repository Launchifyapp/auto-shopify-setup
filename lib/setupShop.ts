...
async function publishProductToOnlineStore(session: Session, productId: string, publicationId: string) {
  ...
  await client.request(query, { variables });
        ...
        // == Publication de tous les produits dans le canal Online Store ==
const onlineStorePublicationId = await getOnlineStorePublicationId(session);
if (onlineStorePublicationId) {
  // On récupère tous les IDs produits dans le shop créés juste avant
  const client = new shopify.clients.Graphql({ session });
  const query = `
  ...
  `;
  const response: any = await client.request(query);
  const productIds = (response?.data?.products?.edges ?? []).map((e: any) => e.node.id);
